...
- `size` / `keyAt` answer the virtualizer's height and key probes without building a row, and are required because a count change re-runs both for every index — never route them through `rowAt`. `size` is a constant for a uniform list, or `{ at, key }` where `key` changes whenever a height would: `count` staying put does **not** mean heights did, and a stale `key` leaves every offset below the change wrong. The container reads source fields at access time and never destructures them, so a source whose values change must expose them as individual getters on one stable object.
