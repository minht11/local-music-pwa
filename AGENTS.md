...
- `sizeAt` / `keyAt` answer the virtualizer's height and key probes without building a row, and are required because a count change re-runs both for every index — never route them through `rowAt`. The container reads source fields at access time and never destructures them, so a source whose values change must expose them as individual getters on one stable object.
