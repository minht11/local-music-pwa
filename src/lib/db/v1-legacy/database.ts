...
interface V1LegacyAppDB extends DBSchema {
	'APP_DATA-1': {
		key: `data-${V1LegacyDatabaseKey}`
		value: V1LegacyTrack[] | V1LegacyPlaylist[] | string[]
	}
}
...
export const getV1LegacyDatabase = (onUpgraded?: (db: IDBPDatabase<V1LegacyAppDB>) => void) =>
	openDB<V1LegacyAppDB>(V1_LEGACY_DB_NAME, 1, {
		upgrade(db) {
			onUpgraded?.(db)
		},
	})
