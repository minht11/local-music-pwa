<script lang="ts">
	...
	export let value =
		typeof initialValue === 'string' ? Number.parseInt(initialValue, 10) : initialValue
	...
	mouseEventShield.addEventListener('pointerdown', (e) => {
		e.preventDefault()
		e.stopPropagation()
	})
	...
	const ro = new ResizeObserver(([entry]) => {
		...
		elementX = entry.contentRect.left
	})
	...
	function onDragStart(e) {
		...
		if (e.type === 'pointerdown') document.body.append(mouseEventShield)
		currentThumb = thumb
	}
...
<svelte:window
	ontouchmove|nonpassive={updateValueOnEvent}
	ontouchcancel={onDragEnd}
	ontouchend={onDragEnd}
	onpointerdown={updateValueOnEvent}
	...
	>
		<div class="range__track" bind:this={container}>
			...
			<div
				...
				onpointerover={() => (thumbHover = true)}
				onpointerout={() => (thumbHover = false)}
			>
