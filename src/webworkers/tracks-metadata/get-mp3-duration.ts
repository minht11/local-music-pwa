...
// Required buffer implementation which is not existant in browser
// also lacked ts declaration type.
...
function frameSize(
  sample: number, layer: string, bitRate: number, sampleRate: number, paddingBit: number,
) {
  if (layer === '1') {
...
export const getMP3Duration = (buffer: ArrayBuffer) => {
  let scratch = new Uint8Array(buffer.slice(0, 100))
  ...
  while (offset < buffer.byteLength) {
    scratch = new Uint8Array(buffer.slice(offset, offset + 10))
